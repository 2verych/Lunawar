...
Админка тоже работает через long polling и всегда показывает актуальное состояние.
...
- Протокол обновлений: Long Polling.
- Деплой: Docker + Nginx.
...
6. **Long Polling**
   ...
   - Создать API `POST /lp/subscribe`:
     - Параметры: cursor, lastEventId.
     - Сервер держит соединение до события или 25 секунд.
     - Возвращает список событий и новый cursor.
...
8. **Чат**
   ...
   - LP-событие `chat.message` для всех участников комнаты.
