...
## Назначение
Этот документ описывает публичные REST и long polling контракты для движка браузерной многопользовательской игры. Цель - чтобы фронт и сервер договорились о формате запросов, ответов и событий. Всё ниже про базовый MVP.

---

## Транспорт
- REST для команд и чтения снимков состояния.
- Long polling для событий реального времени.
- Ответы только JSON.
...
```json
{
  "since": { "epoch": 3, "lastEventId": 1017 },
  "channels": ["user", "room", "lobby", "admin"]
}
```
- Сервер держит соединение до первого события или до таймаута 25 сек.
- Ответ 200:
```json
{
  "events": [
  ...
  "cursor": { "epoch": 3, "lastEventId": 1018 }
}
```
